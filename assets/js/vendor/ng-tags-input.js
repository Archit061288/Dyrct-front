            ...
            scope.eventHandlers = {
                input: {
                    keydown: function($event) {
                        if(scope.text && scope.text.length > (parseInt(options.maxLength)-1) && ($event.keyCode!= 8 && $event.keyCode!= 37 && $event.keyCode!= 39 && $event.keyCode!= 32 && $event.keyCode!= 13)){
                            console.log(scope.text)
                            $event.preventDefault();
                            //scope.text=scope.text.substr(0,(parseInt(options.maxLength)-1));
                        }
                        else{
                            
                            if($event.keyCode== 8)
                            {
                                scope.myvar=scope.text.length-1;
                            }
                            else
                            {
                                if($event.keyCode== 32)
                                ...
                                }else
                                {
                                    ...
                                    else if(scope.text && scope.text.length)
                                    {
                                        scope.textlen=scope.text.length;
                                    }
                                    scope.myvar=scope.textlen+1; 
                                }
                               
                            }
                            if(scope.myvar>=1 && scope.myvar<3)
                            {
                                scope.$emit("invalidfoldertag");
                            }
                            else
                            {
                                scope.$emit("validfoldertag");
