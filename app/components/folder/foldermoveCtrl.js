define(['angularAMD','components/groups/deletegroupCtrl'], function(angularAMD) {
    ...
    angularAMD.controller('foldermoveCtrl', ['$scope', '$rootScope','dyrctservice', '$state', '$stateParams', '$uibModalInstance','$cookieStore','modifires','$filter','folderdata','movefolderid','fldrname','flag','$window','moveshareid','currentfolderdata','showsubfolder','subfolderdata','currentfolderbyparam','$timeout','GLOBALS','$uibModal',
        function($scope, $rootScope,dyrctservice, $state, $stateParams, $uibModalInstance,$cookieStore,modifires,$filter,folderdata,movefolderid,fldrname,flag,$window,moveshareid,currentfolderdata,showsubfolder,subfolderdata,currentfolderbyparam,$timeout,GLOBALS,$uibModal) {
...
//                    $('.modal-body').removeClass('transptant-overlay').fadeOut(3000);
                ...
                modalInstance.result.then(function(selectedItem) {
                     //console.log("cid")
                        ...
                        if(success.data.message=="CID/POI Moved Successfully")
                        {
                            var modalInstance = $uibModal.open({
                    animation: true,
                    size: 'sm',
                    ...
                    resolve: {
                        displaystr: function() {
                            return 'CID/POI Moved Successfully';
                        }
                    }
                });
                modalInstance.result.then(function(selectedItem) {
